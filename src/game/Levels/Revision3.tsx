...
export const Revision3 = (() => {
  try {
    return new ReplacePlantsByStringReplaceLevel({
      title: "",
      titleToken: "Revision 3",
      titleDescription: () => (
        <>Let&apos;s recap the previous lessons one more time</>
      ),
      task: () => (
        <>
          Replace with blackberry{" "}
          <HoverablePlantHint
            hightlightedInds={Revision3.matchedBySolutionPlantInds}
          >
            every plant
          </HoverablePlantHint>{" "}
          that is not following directly AND is not followed directly by rose
          AND is not rose itself
        </>
      ),
      description: "",
      possibleSolution: /(?<!\brose\b\s*)\b\w+\b(?!\s*rose\b)(?<!\brose\b)/gi,
      validators: [
